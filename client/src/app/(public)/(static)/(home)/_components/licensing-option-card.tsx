...
export const LicensingOptionCard = ({
  ...
  return (
    <div className={`p-px rounded-[20px] ${option.border}`}>
      <div
        className={` ${option.background} flex-1 px-6 py-8 rounded-[20px] transition-all hover:shadow-[0_2px_20px_0_#B668F080]`}
      >
        <h3 className="font-semibold text-[30px] leading-[43px] mb-4 first-letter:uppercase">
          {option.title}
        </h3>
        <p
          className={`font-normal text-base mb-6 ${option.tierType === PlanType.EarlyBird ? "text-[#CE9DF3]" : ""}`}
        >
          {option.description}
        </p>
        ...
        <ul className="space-y-[10px] gap-[16px]">
          {option.ulText}
          {option.features.map((feature, index) => (
            <li key={index} className="flex items-center gap-[4px] ">
              <Image
                ...
                alt="Checkmark"
              />
              <span className="font-normal text-[16px]">{feature}</span>
            </li>
          ))}
        </ul>
